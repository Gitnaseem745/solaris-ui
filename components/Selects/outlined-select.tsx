...
const OutlinedSelect: React.FC<OutlinedSelectProps> = ({
  ...
  return (
    <div className="relative w-64">
      <button
        type="button"
        className="w-full px-4 py-2 text-left bg-transparent border-2 border-zinc-300 rounded-md focus:outline-none focus:border-zinc-500"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.ul
            ...
            className="absolute z-10 w-full mt-1 bg-background border-2 border-zinc-300 rounded-md shadow-lg max-h-60 overflow-auto"
          >
            {options.map((option) => (
              <li
                key={option.value}
